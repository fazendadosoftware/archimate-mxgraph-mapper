...
const { jwtClaims, factSheetIndex } = useWorkspace()
const props = defineProps({ row: { type: Object, required: true } })
const { row } = toRefs(props)

const openFactSheet = () => {
  const { factSheet = null } = unref(row)
  const { instanceUrl = null, principal: { permission: { workspaceName } = {} as any } = {} } = unref(jwtClaims) ?? {}
